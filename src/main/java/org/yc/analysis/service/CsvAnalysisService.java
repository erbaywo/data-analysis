...
public class CsvAnalysisService {
     ...
     * 解析账号数据CSV文件
     */
    public List<AccountData> parseAccountCsv(MultipartFile file, String dataDate) {
        ...
        try (CSVReader reader = new CSVReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8))) {
            ...
            if (allRows.size() < 4) {
                throw new BusinessException("CSV文件格式不正确，行数不足");
            }
            ...
            for (int i = 3; i < allRows.size(); i++) {
                ...
                AccountData accountData = buildAccountData(row, dataDate);
                accountDataList.add(accountData);
            }

        } catch (IOException | CsvException e) {
            log.error("解析账号数据CSV文件失败", e);
     ...
     * 构建账号数据对象
     */
    private AccountData buildAccountData(String[] row, String dataDate) {
        ...
        accountData.setCreatedAt(LocalDateTime.now());
        return accountData;
    }
     ...
     * 解析视频数据CSV文件
     */
    public List<VideoData> parseVideoCsv(MultipartFile file) {
        ...
        try (CSVReader reader = new CSVReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8))) {
            ...
            for (int i = 1; i < allRows.size(); i++) {
                ...
                VideoData videoData = buildVideoData(row);
                videoDataList.add(videoData);
            }

        } catch (IOException | CsvException e) {
            log.error("解析视频数据CSV文件失败", e);
            throw new BusinessException("解析CSV文件失败: " + e.getMessage());
        }
     ...
     * 构建视频数据对象
     */
    private VideoData buildVideoData(String[] row) {
